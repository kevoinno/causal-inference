...
with col2:
    delta = st.number_input(
        "Delta (minimum effect size to detect)",
        value=None,
        placeholder="e.g., 0.1",
        min_value=0.001,
        step=0.01,
...
if st.button("Calculate sample size", type="primary"):
    ...
    if alpha is None or power is None or delta is None or variance is None:
        st.error("Please fill in all parameters before calculating.")
    else:
        try:
            result = calculate_sample_size(alpha, power, delta, variance)
            st.write(f"The sample size required for each group is: {result}")
